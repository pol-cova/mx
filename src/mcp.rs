...
impl Server {
    ...
    #[tool(
        description = "Plan, apply, verify, or restore an experimental capability profile on an Mx-owned iOS 26.5 clone. Apply/restore require a stopped app session and reboot the clone. Slim preset disables 172 audited services minus keep categories/capabilities, then removes optional Watch synchronization agents unless the watch capability is retained. Balanced disables 11. Networking and core services are retained."
    )]
    async fn mx_profile(
        &self,
        context: RequestContext<RoleServer>,
